...
function package {
    ...
    if [[ -f "/etc/redhat-release" ]]; then
        ...
        [[ "$centosMajorVersion" == "7" ]] && tagetPackageName="nebula-storage-${version}.el7-5.x86_64.rpm"
        [[ "$centosMajorVersion" == "6" ]] && tagetPackageName="nebula-storage-${version}.el6-5.x86_64.rpm"
    elif [[ -f "/etc/lsb-release" ]]; then
        ubuntuVersion=`cat /etc/lsb-release | grep DISTRIB_RELEASE | cut -d "=" -f 2 | sed 's/\.//'`
        tagetPackageName="nebula-storage-${version}.ubuntu${ubuntuVersion}.amd64.deb"
        pType="DEB"
    fi
