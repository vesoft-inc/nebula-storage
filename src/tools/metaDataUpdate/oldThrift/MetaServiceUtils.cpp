...
std::vector<cpp2::HostAddr> MetaServiceUtils::parsePartVal(folly::StringPiece val) {
...
cpp2::HostAddr MetaServiceUtils::parseHostKey(folly::StringPiece key) {
    cpp2::HostAddr host;
    memcpy(&host, key.data() + kHostsTable.size(), sizeof(host));
    return host;
}

cpp2::HostAddr MetaServiceUtils::parseLeaderKey(folly::StringPiece key) {
    cpp2::HostAddr host;
...
cpp2::IndexItem MetaServiceUtils::parseIndex(const folly::StringPiece& rawData) {
    cpp2::IndexItem item;
