...
struct HostItem {
    1: HostAddr      hostAddr,
    2: HostStatus           status,
    3: map<string, list<PartitionID>> (cpp.template = "std::unordered_map") leader_parts,
    4: map<string, list<PartitionID>> (cpp.template = "std::unordered_map") all_parts,
}


struct PartItem {
    1: required PartitionID       part_id,
    2: optional HostAddr          leader,
    3: required list<HostAddr>    peers,
    4: required list<HostAddr>    losts,
}
