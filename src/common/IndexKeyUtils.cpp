...
void IndexKeyUtils::indexRaw(const IndexValues &values, std::string& raw) {
    ...
    for (auto& col : values) {
        if (col.first == nebula::meta::cpp2::PropertyType::STRING) {
...
// static
std::string IndexKeyUtils::vertexIndexKey(PartitionID partId, IndexID indexId, VertexID vId,
...
// static
std::string IndexKeyUtils::edgeIndexKey(PartitionID partId, IndexID indexId,
