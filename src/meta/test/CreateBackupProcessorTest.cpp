...
TEST(ProcessorTest, CreateBackupTest) {
...

    {
        ...
        auto& metaBackupInfo = meta.get_backup_info();
        ASSERT_EQ(1, metaBackupInfo.size());
        for (auto& [space, backupInfo] : metaBackupInfo) {
            ASSERT_EQ(1, space);
            ASSERT_EQ(1, backupInfo.get_cp_dirs().size());
            auto checkInfo = backupInfo.get_cp_dirs()[0];
