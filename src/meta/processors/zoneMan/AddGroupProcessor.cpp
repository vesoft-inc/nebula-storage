...
void AddGroupProcessor::process(const cpp2::AddGroupReq& req) {
    ...
    if (zoneNames.size() != zoneSet.size()) {
        ...
        handleErrorCode(nebula::cpp2::ErrorCode::E_ZONE_CONFLICT);
        onFinished();
        return;
    }
    ...
    if (nebula::ok(groupRet)) {
        ...
        handleErrorCode(nebula::cpp2::ErrorCode::E_GROUP_EXISTED);
        onFinished();
        return;
    } else {
...
AddGroupProcessor::checkGroupRedundancy(std::vector<std::string> zones) {
    ...
    while (iter->valid()) {
        ...
        if (zones == zoneNames) {
            ...
            return nebula::cpp2::ErrorCode::E_GROUP_EXISTED;
        }
        iter->next();
    }
