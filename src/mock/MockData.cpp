...
std::shared_ptr<meta::NebulaSchemaProvider> MockData::mockServeSchema(SchemaVer ver) {
    ...
    schema->addField(std::move(col2.name), std::move(col2.type), 0, false, "");

    meta::cpp2::ColumnDef col3;
    col3.name = "startYear";
    col3.type = meta::cpp2::PropertyType::INT64;
    schema->addField(std::move(col3.name), std::move(col3.type), 0, false, 2020L);

    meta::cpp2::ColumnDef col4;
    col4.name = "endYear";
    col4.type = meta::cpp2::PropertyType::INT64;
    schema->addField(std::move(col4.name), std::move(col4.type), 0, false, 2020L);

    meta::cpp2::ColumnDef col5;
    col5.name = "teamCareer";
    col5.type = meta::cpp2::PropertyType::INT64;
    schema->addField(std::move(col5.name), std::move(col5.type), 0, false, 1L);

    meta::cpp2::ColumnDef col6;
    col6.name = "teamGames";
    col6.type = meta::cpp2::PropertyType::INT64;
    schema->addField(std::move(col6.name), std::move(col6.type), 0, false, 1L);

    meta::cpp2::ColumnDef col7;
    col7.name = "teamAvgScore";
    col7.type = meta::cpp2::PropertyType::DOUBLE;
    schema->addField(std::move(col7.name), std::move(col7.type), 0, false, 0.0);
