...
public:
    ...
    kvstore::ResultCode execute(PartitionID partId, const VertexID& vId) override {
        ...
        for (; filterNode_->valid(); filterNode_->next(), ++edgeRowCount) {
            ...
            ret = collectEdgeProps(edgeType, key, reader, props, row, columnIdx, stats);
            if (ret != kvstore::ResultCode::SUCCEEDED) {
                return ret;
            }
        }
...
private:
    ...
    kvstore::ResultCode collectEdgeProps(EdgeType edgeType,
        ...
        if (result.columns[idx].type() == Value::Type::NULLVALUE) {
            result.columns[idx].setList(nebula::List());
        }
        auto& cell = result.columns[idx].mutableList();
        cell.values.emplace_back(std::move(row));
        return kvstore::ResultCode::SUCCEEDED;
    }
