...
void TransactionManager::markDanglingEdge(GraphSpaceID spaceId, const std::string& edge) {
    auto key = std::to_string(spaceId) + edge;
    danglingEdges_.insert(std::make_pair(key, 0));
}

bool TransactionManager::takeDanglingEdge(GraphSpaceID spaceId, const std::string& edge) {
    auto key = std::to_string(spaceId) + edge;
    if (danglingEdges_.find(key) == danglingEdges_.cend()) {
        return false;
    }
    danglingEdges_.erase(key);
    return true;
}
