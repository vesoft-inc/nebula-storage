...
public:
    ...
    bool connectToMeta(const std::string& metaName, int32_t metaPort) {
        ...
        iClient_ = std::make_unique<InternalStorageClient>(executor_, mClient_.get());
...
public:
    ...
    int32_t getPartId(const std::string& src) {
        auto partIdStatus = mClient_->partId(spaceId_, src);
        LOG_IF(FATAL, !partIdStatus.ok()) << "mClient_->partId() failed";
        return partIdStatus.value();
    }
    ...
    std::string encodeProps(const cpp2::NewEdge& e) {
        ...
        auto pSchema = env_->schemaMan_->getEdgeSchema(spaceId_, std::abs(edgeType)).get();
